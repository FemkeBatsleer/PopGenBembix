...
RM_P <- read.csv2(file = "Results_indassign_allpops_RM_probRM10000MCalpha001_forR.csv", header=T) %>%
...
write.csv(RM_P_f, file="Outputs for QGIS visualisations/all pops/list_assigned_all.csv")
...
edges_pops_RM <- connections_pops_df_RM %>% rename("to"="pop", "from"="pop_max", "intensity"="n") %>%
  ...
  left_join(metadatapop %>% dplyr::select(Population, Number_pop, Region) %>% rename("from"="Population", "idfrom"="Number_pop", "regionfrom"="Region"), #add id's of from population
            by="from")
write.csv(edges_pops_RM, file="Outputs for QGIS visualisations/all pops/edges_pops_RM_all.csv") #write to csv file

# #write a file for the nodes, from metadatapop
write.csv(metadatapop, file="Outputs for QGIS visualisations/all pops/nodes_pops_all.csv")
...
write.csv(edges_pops_RM %>%
          ...
          file="Outputs for QGIS visualisations/all pops/edges_pops_RM_fromcoast_all.csv")
write.csv(edges_pops_RM %>%
            dplyr::filter((regionfrom=="Flanders-inland"|regionfrom=="Wallonia") ),
          file="Outputs for QGIS visualisations/all pops/edges_pops_RM_frominland_all.csv")
...
write.csv(edges_pops_RM %>% dplyr::filter((regionto=="Flanders-coast"|regionto=="France-Picardie") &
          ...
          file="Outputs for QGIS visualisations/all pops/edges_pops_RM_withincoast.csv")
#within inland
write.csv(edges_pops_RM %>% dplyr::filter((regionto=="Flanders-inland"|regionto=="Wallonia") &
          ...
          file="Outputs for QGIS visualisations/all pops/edges_pops_RM_withininland.csv")
#from coast to inland
write.csv(edges_pops_RM %>% dplyr::filter((regionto=="Flanders-inland"|regionto=="Wallonia") &
                                            (regionfrom=="Flanders-coast"|regionfrom=="France-Picardie") ),
          file="Outputs for QGIS visualisations/all pops/edges_pops_RM_fromcoasttoinland.csv")
#from inland to coast
write.csv(edges_pops_RM %>% dplyr::filter((regionto=="Flanders-coast"|regionto=="France-Picardie") &
                                            (regionfrom=="Flanders-inland"|regionfrom=="Wallonia") ),
          file="Outputs for QGIS visualisations/all pops/edges_pops_RM_frominlandtocoast.csv")
...
sampled_summarised_RM <- RM_P_f %>%
  ...
  group_by(Region_sampled) %>%
  
  summarise( n_all=n(), n_home = sum(home), n_homeregion=sum(home_region),
            n_other = sum(other_region), n_other_coast = sum(other_coast),
            n_other_inland = sum(other_inland)) %>%
  mutate(ratio_pop = n_home/n_all, ratio_region = n_homeregion/n_all,
         ratio_other_coast = n_other_coast/n_all, ratio_other_inland = n_other_inland/n_all,
...
#                                  Region_max %in% c("Flanders-inland", "Wallonie"), 1, 0)) %>%
#   group_by(Region_sampled) %>%
#   summarise(n_other_coast = sum(other_coast), n_other_inland = sum(other_inland),
